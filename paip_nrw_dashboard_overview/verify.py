...
# --- LIPS behaviour -------------------------------------------------------
print("\n=== LIPS ===")
# LIPS is now a single measure: the water a repair would actually recover.
W = {"physical_loss_m3": 100}
...
# LIPS must BE the recoverable-volume order, exactly.
ok("LIPS order equals recoverable-volume order",
   spearmanr(py.lips, py.physical_loss_m3).statistic, 1.0, 1e-9)
ok("LIPS rank 1 is the largest recoverable volume",
   float(py.nsmallest(1, "lips_rank").physical_loss_m3.iloc[0]),
   float(py.physical_loss_m3.max()), 1.0)

# No non-volume term may influence the score. Loss RATE in particular must not:
# ranking by a ratio is the mistake the dashboard exists to correct.
_rate_rho = spearmanr(py.lips, py.nrw_pct).statistic
checks += 1
_ok = abs(_rate_rho) < 0.95
print(f"  {'PASS' if _ok else 'FAIL'}  "
      f"{'LIPS is not driven by loss rate':52s} {_rate_rho:>18.3f} rho")
if not _ok:
    fails.append("LIPS still tracks loss rate")

# physical_loss is algebraically nrw x share, so weighting share separately
# would double-count it. Confirm the identity that makes it redundant.
ok("physical_loss == nrw x physical_share (share is redundant)",
   (py.physical_loss_m3 - py.nrw_m3 * py.physical_share_pct / 100).abs().max(),
   0.0, 1.0)

# The whole point of the change: volume-ranked top 10 must capture far more
# recoverable water than a rate-ranked one.
_topv = py.nsmallest(10, "lips_rank").physical_loss_m3.sum() / py.physical_loss_m3.sum()
_topr = py.nsmallest(10, "rate_rank").physical_loss_m3.sum() / py.physical_loss_m3.sum()
checks += 1
print(f"  {'PASS' if _topv > _topr * 2 else 'FAIL'}  "
      f"{'volume top-10 beats rate top-10 on water':52s} "
      f"{_topv*100:>10.1f}% vs {_topr*100:.1f}%")
if _topv <= _topr * 2:
    fails.append("volume ranking no longer dominates on recoverable water")
