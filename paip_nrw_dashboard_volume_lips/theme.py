...
<style>
  ...
  /* Streamlit's floating header overlays the first ~55px and was clipping the
     brand row. It only carries the Deploy button and the burger menu, neither
     of which this dashboard needs, so it is removed and the reclaimed height
     goes to content. max-width is widened for the 1920px target. */
  ...
  /* Top strip ---------------------------------------------------------
     The dashboard has no sidebar and must not scroll on a 1080p screen, so
     the brand block and KPI row are compressed into fixed-height strips. */
