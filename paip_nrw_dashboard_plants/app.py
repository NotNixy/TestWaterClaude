...
with TAB_OVERVIEW:
    ...
    with _sub[0]:
        ...
        with c1:
            ...
            # Plant level, not district: a district is an administrative unit,
            # not something a crew is dispatched to. 74 plants will not fit
            # legibly on one axis, so this shows the largest losers and states
            # how much of the estate's water they account for.
            n_top = min(15, n_plants)
            pl = (sel.nlargest(n_top, "nrw_m3")
                     .sort_values("nrw_m3", ascending=True))
            share_shown = pl.nrw_m3.sum() / sel.nrw_m3.sum() * 100

            fig = go.Figure(go.Bar(
                x=pl.nrw_m3, y=pl.plant, orientation="h",
                marker=dict(color=pl.nrw_pct, colorscale=T.SEQ,
                ...
                text=[m3(v) for v in pl.nrw_m3], textposition="outside",
                textfont=dict(size=11, color=T.INK_2),
                customdata=np.stack([pl.district, pl.nrw_pct,
                                     pl.physical_loss_m3, pl.nrw_value_rm], -1),
                hovertemplate=("<b>%{y}</b> · %{customdata[0]}<br>"
                               "NRW volume  %{x:,.0f} m³<br>"
                               "Loss rate  %{customdata[1]:.1f}%<br>"
                               "Recoverable by repair  %{customdata[2]:,.0f} m³<br>"
                               "Value  RM %{customdata[3]:,.0f}<extra></extra>")))
            fig.update_layout(
                title=f"NRW volume by plant — {n_top} largest, shaded by loss rate",
                height=555, bargap=0.3,
                xaxis=dict(title="NRW volume (m³)",
                           range=[0, pl.nrw_m3.max() * 1.16]),
                yaxis=dict(title=None, tickfont=dict(size=10.5)))
            st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
            st.markdown(
                f'<div class="caption">Colour carries the loss rate, length '
                f'carries the volume. These {n_top} plants hold '
                f'<b>{share_shown:.0f}%</b> of all non-revenue water in the '
                f'current selection. Note how little the two encodings agree: '
                f'the longest bars are rarely the darkest, which is the '
                f'rate-versus-volume problem visible in a single chart.</div>',
                unsafe_allow_html=True)
    ...
    with _sub[1]:
        ...
        # The headline chart is now plant-level, so the district rollup lives
        # here — it is still the right unit for a regional summary, just not for
        # deciding where to send a crew.
        with st.expander("District rollup"):
            dist = (sel.groupby("district", as_index=False)
                       ...
                       .assign(nrw_pct=lambda d: d.nrw_m3 / d.production_m3 * 100)
                       .sort_values("nrw_m3", ascending=False))
            st.dataframe(
                dist[["district", "plants", "production_m3", "nrw_m3",
                      "physical_loss_m3", "nrw_pct", "value_rm"]],
