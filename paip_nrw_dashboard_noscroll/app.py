...
# LIPS weights are set inside the Priority Schedule view, which is the only
# place they apply. They are seeded here so every view can score consistently.
weights = {c: st.session_state.get(f"w_{c}", d)
           for c, d in DEFAULT_WEIGHTS.items()}
blend_crit = st.session_state.get("blend_crit", False)
crit_weight = st.session_state.get("crit_weight", 20)
...
# ==========================================================================
with TAB_OVERVIEW:
    _sub = st.tabs(["Where the water goes", "Trajectory"])
    with _sub[0]:
        ...
        with c4:
            ...
            fig.add_trace(go.Bar(
                ...
                hovertemplate=("<b>%{x}</b><br>Loss rate  %{y:.2f}%<br>"
                               ...
                               "<extra></extra>")))
            fig.update_layout(
                title="Loss rate against national reference points", height=520,
...
# ==========================================================================
with TAB_RATEVOL:
    _sub = st.tabs(["The divergence", "The two queues"])
    with _sub[0]:
...
# ==========================================================================
with TAB_SCHEDULE:
    _sub = st.tabs(["Ranking", "Weights", "Recovery curve", "Full schedule",
                    "Criticality blend"])

    with _sub[1]:
        # The weights only affect this view's score, so they live here rather
        # than in a global strip. Changing one re-runs the app and every LIPS
        # figure updates.
        st.markdown("###### Component weights")
        st.markdown('<div class="caption">Relative importance of each '
                    'component. Values are renormalised to sum to 100, so only '
                    'the ratios matter. Set a weight to 0 to drop a component '
                    'entirely.</div>', unsafe_allow_html=True)
        _wc = st.columns(3)
        for _i, (_col, _default) in enumerate(DEFAULT_WEIGHTS.items()):
            _label, _help = LIPS_COMPONENTS[_col]
            with _wc[_i % 3]:
                st.slider(_label, 0, 50, _default, 1, key=f"w_{_col}",
                          help=_help)
        if HAS_ML:
            st.markdown("---")
            _bc = st.columns([1, 1, 1])
            with _bc[0]:
                st.checkbox("Fold Criticality into LIPS", key="blend_crit",
                            help="Adds the model-based Criticality Index as a "
                                 "seventh component. Off by default so the "
                                 "deterministic and model-based scores stay "
                                 "comparable.")
            with _bc[1]:
                st.slider("Criticality weight", 0, 50, 20, 1, key="crit_weight",
                          disabled=not st.session_state.get("blend_crit", False))
            with _bc[2]:
                st.markdown('<div class="caption">Compare the two orderings in '
                            'the <b>Criticality blend</b> view.</div>',
                            unsafe_allow_html=True)
    ...
    with _sub[0]:
        ...
        with c1:
            ...
            fig.update_layout(
                title=f"Top {n_show} plants by intervention priority", height=470,
        ...
        with c2:
            ...
            fig.update_layout(
                title="LIPS decomposition — contribution to score", height=470,
    ...
    with _sub[4]:
        if not (ML_MATCHES_YEAR and blend_crit and crit_weight > 0):
            st.info("Enable **Fold Criticality into LIPS** under Weights to "
                    "compare the blended and unblended rankings here.")
        else:
...
# ==========================================================================
# TAB 4 — Burst Risk (supervised prediction)
# ==========================================================================
with TAB_BURST:
    if not HAS_BURST:
    ...
    with _sub[1]:
            ...
            if len(d) > 2:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hoverinfo="skip"))
            fig.update_layout(
                title=f"Loss rate against {dlabel.lower()}", height=610,
