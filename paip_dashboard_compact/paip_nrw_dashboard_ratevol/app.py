...
with st.sidebar:
    ...
    # Empty means "all". Defaulting these to every value selected produced a
    # wall of chips tall enough to push the rest of the sidebar off-screen —
    # the filter was costing more room than the charts it scoped.
    _r_all = sorted(yearly.region.unique())
    regions = st.multiselect("Region", _r_all, default=[],
                             placeholder="All regions") or _r_all
    _d_all = sorted(yearly[yearly.region.isin(regions)].district.unique())
    districts = st.multiselect("District", _d_all, default=[],
                               placeholder="All districts") or _d_all
    _a_all = sorted(yearly.area_type.unique())
    areas = st.multiselect("Area type", _a_all, default=[],
                           placeholder="All area types") or _a_all
...
tabs = st.tabs(["Command", "Overview", "Loss Dynamic", "Burst Risk",
                "Plant Profile"])
(TAB_CMD, TAB_OVERVIEW, TAB_RATEVOL, TAB_BURST, TAB_PLANT) = tabs


def mini(fig, title, h=262, ylab=None, xlab=None, legend=False):
    """Shrink a figure to card size and put its title INSIDE the plot.

    The title belongs in the figure rather than in a markdown element above it
    because Streamlit collapses a markdown container that precedes a chart to a
    few pixels, and the chart then paints over the heading. Keeping it inside
    also costs less vertical space, which is the whole currency of this screen.
    """
    fig.update_layout(
        height=h, margin=dict(l=4, r=8, t=30 if not legend else 46, b=4),
        ...
        legend=dict(orientation="h", yanchor="bottom", y=1.0, x=0,
                    font=dict(size=9)),
        font=dict(size=9.5),
        xaxis=dict(title=xlab, title_font=dict(size=9.5), tickfont=dict(size=9)),
        yaxis=dict(title=ylab, title_font=dict(size=9.5), tickfont=dict(size=9)))
    return fig
...
# ==========================================================================
with TAB_CMD:
    # Nine cards on one screen: every argument the dashboard makes, visible at
    # a glance. Each is a stripped-down version of a chart that has a
    # full-size, fully-explained counterpart in one of the sections behind it.
    _n8 = min(8, n_plants)
    r1 = st.columns(3)
    ...
    with r1[0]:
        ...
        f = go.Figure(go.Bar(
            ...
            marker=dict(color=d_.lips, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=1.5), showscale=False),
            text=[f"{v:.0f}" for v in d_.lips], textposition="outside",
            textfont=dict(size=9, color=T.INK_2), customdata=d_.district,
            hovertemplate=("<b>%{y}</b> · %{customdata}<br>"
                           "LIPS %{x:.1f}<extra></extra>")))
        f.update_xaxes(range=[0, 118])
        st.plotly_chart(mini(f, f"Priority queue · top {_n8} by LIPS"),
                        width="stretch", config=PLOT_CFG, theme=None)

    with r1[1]:
        # Four series, so a legend is mandatory; the 2px surface gap between
        # stacked segments does the separating rather than a drawn border.
        d_ = sel.nsmallest(_n8, "lips_rank").sort_values("lips")
        f = go.Figure()
        ...
        st.plotly_chart(mini(f, "What drives each score", legend=True),
                        width="stretch", config=PLOT_CFG, theme=None)

    with r1[2]:
        # Scatter is an all-pairs form, so it is capped at the first three
        # categorical slots.
        tr_ = sel.nsmallest(10, "rate_rank")
        tv_ = sel.nsmallest(10, "volume_rank")
        pl_ = sel.copy()
        pl_["grp"] = np.select(
            [pl_.plant.isin(set(tr_.plant) & set(tv_.plant)),
             pl_.plant.isin(tv_.plant), pl_.plant.isin(tr_.plant)],
            ["Both", "Top 10 volume", "Top 10 rate"], default="Neither")
        f = go.Figure()
        for g_, c_ in [("Neither", T.NEUTRAL), ("Top 10 volume", T.BLUE),
                       ("Top 10 rate", T.ORANGE), ("Both", T.AQUA)]:
            q = pl_[pl_.grp == g_]
            if q.empty:
                continue
            f.add_trace(go.Scatter(
                x=q.production_m3, y=q.nrw_pct, mode="markers", name=g_,
                marker=dict(size=np.sqrt(q.nrw_m3 / pl_.nrw_m3.max()) * 20 + 5,
        ...
        st.plotly_chart(mini(f, "Rate vs volume — the divergence", legend=True),
                        width="stretch", config=PLOT_CFG, theme=None)
    ...
    with r2[0]:
        ...
        f = go.Figure(go.Scatter(
            x=mo.month, y=mo.pct, mode="lines+markers",
        ...
        f.update_yaxes(ticksuffix="%", range=[mo.pct.min() - 1.2,
                                              mo.pct.max() + 1.2])
        f.update_xaxes(dtick=1)
        st.plotly_chart(mini(f, f"Monthly loss rate · {year}"),
                        width="stretch", config=PLOT_CFG, theme=None)
    ...
    with r2[1]:
        ...
        f = go.Figure(go.Bar(
            ...
            marker=dict(color=d_.v, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=1.5), showscale=False),
            hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        st.plotly_chart(mini(f, "Water lost by district"),
                        width="stretch", config=PLOT_CFG, theme=None)
    ...
    with r2[2]:
        ...
        f.add_annotation(x=n10, y=sv.cum.iloc[n10 - 1],
                         ...
                         showarrow=False, xshift=44, yshift=-11,
                         font=dict(size=10, color=T.INK))
        f.update_yaxes(range=[0, 104], ticksuffix="%")
        st.plotly_chart(mini(f, "Loss concentration"),
                        width="stretch", config=PLOT_CFG, theme=None)
    ...
    with r3[0]:
        if HAS_BURST:
            bp = burst_pred.copy()
            bands = [("Low", 0.0, 0.25, T.GOOD), ("Moderate", 0.25, 0.5, T.WARNING),
                     ("High", 0.5, 0.75, T.SERIOUS), ("Critical", 0.75, 1.01, T.CRITICAL)]
            xs = [b[0] for b in bands]
            ys = [int(((bp.risk >= lo) & (bp.risk < hi)).sum())
                  for _, lo, hi, _c in bands]
            f = go.Figure(go.Bar(
                x=xs, y=ys, marker=dict(color=[b[3] for b in bands],
                                        line=dict(color=T.SURFACE, width=1.5)),
                text=ys, textposition="outside",
                textfont=dict(size=9, color=T.INK_2),
                hovertemplate="<b>%{x}</b><br>%{y} plants<extra></extra>"))
            f.update_layout(bargap=0.45)
            f.update_yaxes(range=[0, max(ys) * 1.28 if max(ys) else 1])
            st.plotly_chart(mini(f, "Burst risk bands · next month"),
                            width="stretch", config=PLOT_CFG, theme=None)
        else:
            st.markdown('<div class="card"><div class="card-t">Burst risk'
                        '</div><div class="caption">No burst model artefacts '
                        'in this build.</div></div>', unsafe_allow_html=True)
    ...
    with r3[1]:
        ...
        f.add_trace(go.Bar(
            ...
            name="Physical (assumed)",
            marker=dict(color=T.BLUE, line=dict(color=T.SURFACE, width=1.5)),
            hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        f.add_trace(go.Bar(
            x=d_.commercial_loss_m3, y=d_.plant, orientation="h",
            name="Commercial (assumed)",
            marker=dict(color=T.ORANGE, line=dict(color=T.SURFACE, width=1.5)),
            hovertemplate="<b>%{y}</b><br>%{x:,.0f} m³<extra></extra>"))
        f.update_layout(barmode="stack")
        st.plotly_chart(mini(f, "Loss composition · largest plants",
                             legend=True),
                        width="stretch", config=PLOT_CFG, theme=None)

    with r3[2]:
        if ML_MATCHES_YEAR and "criticality" in sel.columns \
                and sel.criticality.notna().any():
            q = sel.dropna(subset=["criticality"])
            f = go.Figure(go.Scatter(
                x=q.lips, y=q.criticality, mode="markers",
                marker=dict(size=7, color=T.BLUE, opacity=0.8,
                            line=dict(color=T.SURFACE, width=2)),
                customdata=q.plant,
                hovertemplate=("<b>%{customdata}</b><br>LIPS %{x:.1f}"
                               "<br>Criticality %{y:.1f}<extra></extra>")))
            st.plotly_chart(mini(f, "Criticality vs LIPS — different questions",
                                 ylab="Criticality", xlab="LIPS"),
                            width="stretch", config=PLOT_CFG, theme=None)
        else:
            d_ = sel.nlargest(_n8, "bursts_per_100km").sort_values("bursts_per_100km")
            f = go.Figure(go.Bar(
                x=d_.bursts_per_100km, y=d_.plant, orientation="h",
                marker=dict(color=d_.bursts_per_100km, colorscale=T.SEQ,
                            line=dict(color=T.SURFACE, width=1.5),
                            showscale=False),
                hovertemplate="<b>%{y}</b><br>%{x:.1f} per 100 km<extra></extra>"))
            st.plotly_chart(mini(f, "Burst rate · worst plants"),
                            width="stretch", config=PLOT_CFG, theme=None)
