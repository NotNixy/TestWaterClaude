...
k[2].markdown(T.tile("Value at tariff", "RM " + rm(tot_val), "",
                     f"Forgone revenue at RM {sel.tariff_rm_m3.mean():.2f}/m³ average"),
              unsafe_allow_html=True)
k[3].markdown(T.tile("Physical leakage", f"{tot_phys/tot_nrw*100:.0f}", "%",
                     f"{m3(tot_phys)} m³ addressable by pipe repair"),
              unsafe_allow_html=True)
above = int((sel.nrw_pct > T.POLICY_TARGET_PCT).sum())
k[4].markdown(T.tile("Above 25% target", f"{above}", f"of {n_plants}",
...
tabs = st.tabs(["Overview", "Rate vs Volume", "Priority Schedule",
                "Early Warning", "Loss Composition", "Financial Impact",
                "Plant Profile", "Method & Data Quality", "Data Management"])
(TAB_OVERVIEW, TAB_RATEVOL, TAB_SCHEDULE, TAB_EARLY,
 TAB_COMPOSITION, TAB_FINANCE, TAB_PLANT, TAB_METHOD, TAB_DATA) = tabs
...
with TAB_OVERVIEW:
    ...
    with c3:
        ...
        mo["label"] = [names[i - 1] for i in mo.month]

        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=mo.label, y=mo.nrw_pct, mode="lines+markers",
            line=dict(color=T.BLUE, width=2.5),
            marker=dict(size=8, color=T.BLUE,
                        line=dict(color=T.SURFACE, width=2)),
            name="Loss rate",
            customdata=np.stack([mo.nrw_m3, mo.rainfall_mm], -1),
            hovertemplate=("<b>%{x}</b><br>Loss rate  %{y:.2f}%<br>"
                           "Volume  %{customdata[0]:,.0f} m³<br>"
                           "Rainfall  %{customdata[1]:.0f} mm<extra></extra>")))
        span = mo.nrw_pct.max() - mo.nrw_pct.min()
        pad = max(span * 1.8, 2.0)
        mid = mo.nrw_pct.mean()
        fig.update_layout(
            title="Monthly loss rate", height=340, showlegend=False,
            xaxis=dict(title=None),
            yaxis=dict(title="NRW (% of production)", ticksuffix="%",
                       range=[mid - pad, mid + pad]))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown(
            f'<div class="caption">The rate moves within a {span:.2f} pp band '
            f'across the whole year, with no wet-season or dry-season signal. '
            f'Flat losses of this kind point to continuous physical leakage '
            f'rather than seasonal demand swings or intermittent billing '
            f'errors — and continuous leakage is exactly what repair work '
            f'recovers. Axis is zoomed to the data range; the full 0–100% scale '
            f'would render this line perfectly flat.</div>',
            unsafe_allow_html=True)
...
with TAB_EARLY:
    if not HAS_ML:
        st.warning("Model artefacts not found. Run `python train_models.py` "
                   "to generate them, then reload.")
        st.stop()
    if not ML_MATCHES_YEAR:
        # Scores belong to one focus year; showing them under another year's
        # heading would mislabel them.
        st.info(
            f"The early-warning models are fitted for **{ML_YEAR}**, the most "
            f"recent year in the data, but **{year}** is selected. Switch the "
            f"year to {ML_YEAR} to see criticality scores, or re-run "
            f"`python refresh.py` after adding a newer year.")
        st.stop()

    cvres = ml_metrics["cv_grouped_by_plant"]
    best = ml_metrics["selected_model"]
    det = ml_metrics["deterioration_summary"]
    MODEL_NAMES = {"gb": "Gradient boosting", "ridge": "Ridge regression",
                   "mean": "Mean baseline"}

    st.markdown(f"#### Which plants are hiding a fixable defect? · {ML_YEAR}")
    if is_partial(ML_YEAR):
        st.markdown(T.callout(
            f"<b>{ML_YEAR} is still in progress</b> "
            f"({MONTHS_BY_YEAR[ML_YEAR]} of 12 months). Residuals are computed "
            f"on the months observed, so they are directionally usable but "
            f"noisier than a settled year — treat the ranking as provisional "
            f"until the year completes.", "warn"), unsafe_allow_html=True)
    st.markdown(T.callout(
        "This asks a different question from LIPS. LIPS ranks by <b>how much "
        "water</b> is recoverable. This ranks by <b>how wrong a plant looks</b>: "
        "a model predicts what each plant <i>should</i> lose given its pipes, "
        "age, bursts, pressure and terrain, and the plants losing far more than "
        "predicted are carrying loss their network cannot account for. "
        "<b>There are no repair records in this dataset</b>, so nothing here is "
        "a trained failure classifier — it is a model of expected loss, and the "
        "residual is the signal."), unsafe_allow_html=True)

    k = st.columns(5)
    top_c = sel.dropna(subset=["criticality"]).nsmallest(1, "criticality_rank")
    n_unexpl = int((sel.unexplained_pp > 5).sum())
    tot_unexpl = float(sel.unexplained_m3.clip(lower=0).sum())
    k[0].markdown(T.tile(
        "Model R²", f"{cvres[best]['r2']:.2f}", "",
        f"{MODEL_NAMES[best]}, out-of-fold on unseen plants"),
        unsafe_allow_html=True)
    k[1].markdown(T.tile(
        "Typical error", f"{cvres[best]['mae']:.1f}", "pp",
        f"Mean absolute error vs {cvres['mean']['mae']:.1f} pp for a mean baseline"),
        unsafe_allow_html=True)
    k[2].markdown(T.tile(
        "Plants above expectation", f"{n_unexpl}", f"of {n_plants}",
        "Losing more than 5 pp above their predicted rate"),
        unsafe_allow_html=True)
    k[3].markdown(T.tile(
        "Unexplained water", m3(tot_unexpl), "m³",
        f"{tot_unexpl/tot_nrw*100:.0f}% of all NRW is above model expectation"),
        unsafe_allow_html=True)
    k[4].markdown(T.tile(
        "Deteriorating plants", f"{det['plants_with_significant_worsening']}",
        f"of {ml_metrics['n_plants']}",
        "Statistically significant worsening trend — see the null result below"),
        unsafe_allow_html=True)

    st.markdown("")
    st.markdown(T.callout(
        f"<b>The deterioration detectors returned a null result, and that is a "
        f"finding.</b> Across all 74 plants and 36 months: "
        f"<b>{det['plants_with_significant_worsening']}</b> plants show a "
        f"statistically significant worsening trend, "
        f"<b>{det['plants_with_significant_step_increase']}</b> show a "
        f"significant step increase, and only "
        f"<b>{det['total_anomaly_months']}</b> month-level anomalies were "
        f"detected in 2,664 records. The estate is improving uniformly at a "
        f"median <b>{abs(det['median_trend_pp_yr']):.2f} pp per year</b>. "
        f"Criticality here is therefore driven almost entirely by "
        f"<i>unexplained loss</i> — chronic, stable, structural losses rather "
        f"than sudden failures. Ranking is still valid; the sudden-failure "
        f"component simply has nothing to contribute on this data.", "good"),
        unsafe_allow_html=True)

    c1, c2 = st.columns([1.15, 1])
    with c1:
        st.markdown("###### Actual loss against model expectation")
        d = sel.dropna(subset=["expected_nrw_pct"])
        lo = float(min(d.expected_nrw_pct.min(), d.actual_nrw_pct.min())) - 2
        hi = float(max(d.expected_nrw_pct.max(), d.actual_nrw_pct.max())) + 2
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=[lo, hi], y=[lo, hi], mode="lines", name="Exactly as predicted",
            line=dict(color=T.MUTED, width=1.5, dash="dash"), hoverinfo="skip"))
        fig.add_trace(go.Scatter(
            x=d.expected_nrw_pct, y=d.actual_nrw_pct, mode="markers",
            name="Plant",
            marker=dict(size=np.sqrt(d.nrw_m3 / d.nrw_m3.max()) * 30 + 8,
                        color=d.unexplained_pp, colorscale=T.SEQ,
                        opacity=0.88, line=dict(color=T.SURFACE, width=2),
                        colorbar=dict(title=dict(text="Unexplained pp",
                                                 side="right",
                                                 font=dict(size=11, color=T.INK_2)),
                                      ...
                                      ticklen=3, tickcolor=T.BASELINE,
                                      tickfont=dict(size=10, color=T.MUTED))),
            customdata=np.stack([d.plant, d.district, d.unexplained_pp,
                                 d.unexplained_m3], -1),
            hovertemplate=("<b>%{customdata[0]}</b> · %{customdata[1]}<br>"
                           "Expected  %{x:.1f}%<br>Actual  %{y:.1f}%<br>"
                           "Unexplained  %{customdata[2]:+.1f} pp "
                           "(%{customdata[3]:,.0f} m³)<extra></extra>")))
        # Leader lines rather than floating text: the biggest residuals cluster
        # in the same corner, so static offsets collide. Angles alternate so the
        # labels fan out instead of stacking.
        for j, (_, r) in enumerate(d.nlargest(3, "unexplained_pp").iterrows()):
            fig.add_annotation(
                x=r.expected_nrw_pct, y=r.actual_nrw_pct, text=r.plant,
                showarrow=True, arrowhead=0, arrowwidth=1,
                arrowcolor=T.MUTED, ax=[-58, 6, 62][j], ay=[-30, -46, -24][j],
                font=dict(size=10.5, color=T.INK_2), bgcolor=T.SURFACE,
                bordercolor=T.BORDER, borderwidth=1, opacity=0.95, borderpad=3)
        fig.update_layout(
            title="Plants above the diagonal lose more than their network explains",
            height=470,
            xaxis=dict(title="Expected loss rate (%)", ticksuffix="%",
                       range=[lo, hi]),
            yaxis=dict(title="Actual loss rate (%)", ticksuffix="%",
                       range=[lo, hi]))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown(
            '<div class="caption">The diagonal is "exactly as predicted". '
            'Vertical distance above it is the residual — loss that plant age, '
            'network length, burst history, pressure and terrain together do '
            'not account for. That gap is where a survey crew is most likely '
            'to find something.</div>', unsafe_allow_html=True)

    with c2:
        st.markdown("###### Most critical plants")
        n_show = min(14, n_plants)
        crit = sel.dropna(subset=["criticality"]).nsmallest(n_show, "criticality_rank")
        crit = crit.sort_values("criticality")
        fig = go.Figure(go.Bar(
            x=crit.criticality, y=crit.plant, orientation="h",
            marker=dict(color=crit.criticality, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=2), showscale=False),
            text=[f"{v:.0f}" for v in crit.criticality], textposition="outside",
            textfont=dict(size=11, color=T.INK_2),
            customdata=np.stack([crit.district, crit.unexplained_pp,
                                 crit.unexplained_m3, crit.archetype], -1),
            hovertemplate=("<b>%{y}</b> · %{customdata[0]}<br>"
                           "Criticality  %{x:.1f}<br>"
                           "Unexplained  %{customdata[1]:+.1f} pp "
                           "(%{customdata[2]:,.0f} m³)<br>"
                           "Pattern  %{customdata[3]}<extra></extra>")))
        fig.update_layout(
            title="Criticality Index — where something looks wrong", height=470,
            bargap=0.3,
            xaxis=dict(title="Criticality (0–100)",
                       range=[0, min(105, crit.criticality.max() * 1.18)]),
            yaxis=dict(title=None, tickfont=dict(size=11)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        w = ml_metrics["criticality_weights"]
        wt = sum(w.values())
        st.markdown(
            f'<div class="caption">Composite of unexplained loss '
            f'({w["unexplained"]/wt*100:.0f}%), sudden deterioration '
            f'({w["deterioration"]/wt*100:.0f}%) and trend relative to the '
            f'estate ({w["trend"]/wt*100:.0f}%). Because the estate is improving '
            f'uniformly, the trend component measures <i>improving more slowly '
            f'than peers</i> rather than outright worsening.</div>',
            unsafe_allow_html=True)
    ...
    with c3:
        kk = st.select_slider("Number of patterns", options=[2, 3, 4, 5],
                              value=int(ml_metrics["cluster_k"]))
        assign, profile, sil, _ = cluster_at(kk, ML_YEAR)
        sil_now = sil[kk]
        fig = go.Figure(go.Bar(
            x=list(sil.keys()), y=list(sil.values()),
            marker=dict(color=[T.BLUE if kx == kk else T.NEUTRAL
                               for kx in sil.keys()],
                        line=dict(color=T.SURFACE, width=2)),
            text=[f"{v:.2f}" for v in sil.values()], textposition="outside",
            textfont=dict(size=11, color=T.INK_2),
            hovertemplate="k = %{x}<br>Silhouette  %{y:.3f}<extra></extra>"))
        fig.update_layout(
            title="Cluster separation by number of patterns", height=300,
            showlegend=False, bargap=0.45,
            xaxis=dict(title="Number of clusters (k)", tickmode="array",
                       tickvals=list(sil.keys())),
            yaxis=dict(title="Silhouette", range=[0, max(sil.values()) * 1.3]))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown(
            f'<div class="caption">Silhouette peaks at <b>{sil_now:.2f}</b> for '
            f'k={kk}. Values below about 0.25 mean <b>weak separation</b>: this '
            f'estate does not fall into crisp failure types, it varies '
            f'continuously. The groupings are a useful summary of which '
            f'intervention suits which plant, not evidence of discrete '
            f'categories.</div>', unsafe_allow_html=True)

    with c4:
        prof = pd.DataFrame(profile)
        st.markdown("###### Pattern profiles and the intervention each implies")
        st.dataframe(
            prof[["name", "plants", "median_nrw_pct", "median_physical_share",
                  "median_bursts_per_100km", "median_plant_age",
                  "total_nrw_m3", "action"]],
            width='stretch', hide_index=True, height=330,
            column_config={
                "name": "Pattern", "plants": "Plants",
                "median_nrw_pct": st.column_config.NumberColumn("NRW", format="%.1f%%"),
                "median_physical_share": st.column_config.NumberColumn("Physical", format="%.0f%%"),
                "median_bursts_per_100km": st.column_config.NumberColumn("Bursts/100km", format="%.0f"),
                "median_plant_age": st.column_config.NumberColumn("Age yr", format="%.0f"),
                "total_nrw_m3": st.column_config.NumberColumn("Total NRW m³", format="%,d"),
                "action": "Implied intervention"})
    ...
    with c5:
        ...
        for kind in ["gb", "ridge", "mean"]:
            rows.append({
                "Model": MODEL_NAMES[kind] + (" ✓ selected" if kind == best else ""),
                "R² (unseen plants)": cvres[kind]["r2"],
                "MAE pp": cvres[kind]["mae"],
                "RMSE pp": cvres[kind]["rmse"],
                "R² (forward in time)":
                    ml_metrics.get("temporal_holdout_prior_years_to_focus", {})
                    .get(kind, {}).get("r2", float("nan")),
            })
        st.dataframe(pd.DataFrame(rows), width='stretch',
                     hide_index=True,
                     column_config={
                         "R² (unseen plants)": st.column_config.NumberColumn(format="%.3f"),
                         "MAE pp": st.column_config.NumberColumn(format="%.2f"),
                         "RMSE pp": st.column_config.NumberColumn(format="%.2f"),
                         "R² (forward in time)": st.column_config.NumberColumn(format="%.3f")})
        st.markdown(f"""
<div class="caption">

<b>Two validation designs, answering different questions.</b>
<b>Unseen plants</b> is 5-fold GroupKFold split <i>by plant</i>: every prediction
comes from a model that never saw that plant. This is the number the residual
rests on, because it is the only one that makes "unexplained by characteristics"
meaningful. <b>Forward in time</b> trains on the earlier years and tests on the
latest one — but the
same plants appear on both sides, so a flexible model can memorise each plant's
own loss level. Its R² is optimistic and is <b>not</b> used for the residual.

<b>The simpler model won.</b> Ridge beats gradient boosting on unseen plants
({cvres['ridge']['r2']:.2f} vs {cvres['gb']['r2']:.2f}). With 74 plant groups and
largely additive structure, the booster spends its capacity on plant-specific
detail that does not transfer — an unconstrained configuration scored just 0.21.
Selecting on grouped CV rather than defaulting to the more complex algorithm is
what keeps the residual honest.

<b>Both beat the baseline decisively</b> — a mean-only model scores
R² {cvres['mean']['r2']:.2f}, so the features carry real signal.

</div>""", unsafe_allow_html=True)

    with c6:
        st.markdown("###### What the model uses")
        imp = pd.DataFrame(ml_metrics["importances"]).head(12).sort_values("importance")
        fig = go.Figure(go.Bar(
            x=imp.importance, y=imp.feature, orientation="h",
            error_x=dict(type="data", array=imp["std"], color=T.MUTED,
                         thickness=1, width=3),
            marker=dict(color=T.BLUE, line=dict(color=T.SURFACE, width=2)),
            hovertemplate=("<b>%{y}</b><br>Permutation importance  "
                           "%{x:.3f} R²<extra></extra>")))
        fig.update_layout(
            title="Permutation importance on held-out plants", height=430,
            bargap=0.3, xaxis=dict(title="Drop in R² when shuffled"),
            yaxis=dict(title=None, tickfont=dict(size=10.5)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown(
            '<div class="caption">Measured on a held-out plant group, so it '
            'reflects predictive use rather than training fit. <b>Read with '
            'care:</b> capacity, staff count, population and connections all '
            'scale together with plant size, and permutation importance splits '
            'credit unpredictably among correlated features. The safe reading '
            'is that <i>plant size and area type</i> dominate, not that any one '
            'of those columns is individually decisive.</div>',
            unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("#### Full early-warning table")
    cols = ["criticality_rank", "plant", "district", "criticality",
            "actual_nrw_pct", "expected_nrw_pct", "unexplained_pp",
            "unexplained_m3", "trend_pp_yr", "step_shift_pp", "anomaly_months",
            "archetype", "lips_rank"]
    ew = sel.dropna(subset=["criticality"]).sort_values("criticality_rank")[cols]
    st.dataframe(
        ew, width='stretch', hide_index=True, height=420,
        column_config={
            "criticality_rank": st.column_config.NumberColumn("#", width="small"),
            "plant": "Plant", "district": "District",
            "criticality": st.column_config.ProgressColumn(
                "Criticality", min_value=0, max_value=100, format="%.1f"),
            "actual_nrw_pct": st.column_config.NumberColumn("Actual", format="%.1f%%"),
            "expected_nrw_pct": st.column_config.NumberColumn("Expected", format="%.1f%%"),
            "unexplained_pp": st.column_config.NumberColumn("Unexplained", format="%+.1f pp"),
            "unexplained_m3": st.column_config.NumberColumn("Unexplained m³", format="%,d"),
            "trend_pp_yr": st.column_config.NumberColumn("Trend pp/yr", format="%+.2f"),
            "step_shift_pp": st.column_config.NumberColumn("Step pp", format="%+.2f"),
            "anomaly_months": st.column_config.NumberColumn("Anomalies"),
            "archetype": "Pattern",
            "lips_rank": st.column_config.NumberColumn("LIPS rank")})
    st.download_button("Download early-warning table (CSV)",
                       ew.to_csv(index=False),
                       f"paip_early_warning_{year}.csv", "text/csv")
...
with TAB_COMPOSITION:
    ...
    st.markdown("---")
    st.markdown("#### Loss intensity, normalised for network size")
    st.markdown('<div class="caption">Absolute volume favours large plants and '
                'the loss rate favours small ones. These two measures normalise '
                'by physical network and by customer base instead, so plants of '
                'different size become comparable on leakage intensity.</div>',
                unsafe_allow_html=True)

    c3, c4 = st.columns(2)
    with c3:
        top_k = sel.nlargest(min(14, n_plants), "nrw_per_km_m3").sort_values("nrw_per_km_m3")
        fig = go.Figure(go.Bar(
            x=top_k.nrw_per_km_m3, y=top_k.plant, orientation="h",
            marker=dict(color=top_k.nrw_per_km_m3, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=2), showscale=False),
            text=[f"{v:,.0f}" for v in top_k.nrw_per_km_m3],
            textposition="outside", textfont=dict(size=10.5, color=T.INK_2),
            customdata=np.stack([top_k.district, top_k.pipe_length_km], -1),
            hovertemplate=("<b>%{y}</b> · %{customdata[0]}<br>"
                           "%{x:,.0f} m³ per km<br>"
                           "Network  %{customdata[1]:,.0f} km<extra></extra>")))
        fig.update_layout(title="NRW per km of main", height=430, bargap=0.3,
                          xaxis=dict(title="m³ lost per km per year",
                                     range=[0, top_k.nrw_per_km_m3.max() * 1.2]),
                          yaxis=dict(title=None, tickfont=dict(size=10.5)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)

    with c4:
        top_c = sel.nlargest(min(14, n_plants), "loss_per_connection_l_day") \
                   .sort_values("loss_per_connection_l_day")
        fig = go.Figure(go.Bar(
            x=top_c.loss_per_connection_l_day, y=top_c.plant, orientation="h",
            marker=dict(color=top_c.loss_per_connection_l_day, colorscale=T.SEQ,
                        line=dict(color=T.SURFACE, width=2), showscale=False),
            text=[f"{v:,.0f}" for v in top_c.loss_per_connection_l_day],
            textposition="outside", textfont=dict(size=10.5, color=T.INK_2),
            customdata=np.stack([top_c.district, top_c.customer_accounts], -1),
            hovertemplate=("<b>%{y}</b> · %{customdata[0]}<br>"
                           "%{x:,.0f} L per connection per day<br>"
                           "Connections  %{customdata[1]:,.0f}<extra></extra>")))
        fig.update_layout(
            title="Physical loss per connection per day", height=430, bargap=0.3,
            xaxis=dict(title="litres / connection / day",
                       range=[0, top_c.loss_per_connection_l_day.max() * 1.2]),
            yaxis=dict(title=None, tickfont=dict(size=10.5)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown('<div class="caption">A proxy for the Infrastructure Leakage '
                    'Index. A true ILI needs unavoidable-annual-real-loss '
                    'modelling from connection density, mains length and '
                    'pressure, which this dataset does not fully support.</div>',
                    unsafe_allow_html=True)
...
# ==========================================================================
with TAB_FINANCE:
    st.markdown("#### What the losses cost")
    sunk = sel.nrw_sunk_cost_rm.sum()
    rev = sel.billed_revenue_rm.sum()
    opex = sel.opex_rm.sum()
    st.markdown(T.callout(
        "Two different money figures, and they answer different questions. "
        "<b>Forgone revenue</b> values lost water at the average tariff — what "
        "PAIP would have billed had it reached a meter. <b>Sunk production "
        "cost</b> values it at the plant's own cost per m³ — energy, chemicals "
        "and maintenance already spent treating water that was never sold. The "
        "first is the commercial prize; the second is money already gone. "
        "Neither is the same as <i>recoverable</i> value, because repair costs "
        "are not in this dataset.", "warn"), unsafe_allow_html=True)

    k = st.columns(4)
    k[0].markdown(T.tile("Forgone revenue", "RM " + rm(tot_val), "",
                         f"NRW valued at RM {sel.tariff_rm_m3.mean():.2f}/m³ average tariff"),
                  unsafe_allow_html=True)
    k[1].markdown(T.tile("Sunk production cost", "RM " + rm(sunk), "",
                         f"Treatment already spent at RM {sel.cost_per_m3_rm.mean():.2f}/m³"),
                  unsafe_allow_html=True)
    k[2].markdown(T.tile("Forgone vs billed revenue", f"{tot_val/rev*100:.0f}", "%",
                         f"Against RM {rm(rev)} actually billed"),
                  unsafe_allow_html=True)
    k[3].markdown(T.tile("Sunk cost vs opex", f"{sunk/opex*100:.0f}", "%",
                         f"Share of RM {rm(opex)} operating spend on unsold water"),
                  unsafe_allow_html=True)
    ...
    with c1:
        n_show = min(16, n_plants)
        fin = sel.nlargest(n_show, "nrw_value_rm").sort_values("nrw_value_rm")
        fig = go.Figure()
        fig.add_trace(go.Bar(
            x=fin.nrw_sunk_cost_rm, y=fin.plant, orientation="h",
            name="Sunk production cost",
            marker=dict(color=T.BLUE, line=dict(color=T.SURFACE, width=2)),
            hovertemplate="<b>%{y}</b><br>Sunk cost  RM %{x:,.0f}<extra></extra>"))
        fig.add_trace(go.Bar(
            x=fin.nrw_value_rm - fin.nrw_sunk_cost_rm, y=fin.plant,
            orientation="h", name="Additional margin forgone",
            marker=dict(color=T.ORANGE, line=dict(color=T.SURFACE, width=2)),
            customdata=fin.nrw_value_rm,
            hovertemplate=("<b>%{y}</b><br>Margin forgone  RM %{x:,.0f}<br>"
                           "Total value  RM %{customdata:,.0f}<extra></extra>")))
        fig.update_layout(
            title="Value of water lost — production cost and forgone margin",
            height=560, barmode="stack", bargap=0.3,
            xaxis=dict(title="RM per year"),
            yaxis=dict(title=None, tickfont=dict(size=11)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown('<div class="caption">The blue segment is money already '
                    'spent producing water that was never sold; the orange is '
                    'the margin on top that a sale would have earned. Together '
                    'they are the tariff value of the loss.</div>',
                    unsafe_allow_html=True)

    with c2:
        st.markdown("###### Value at stake against priority")
        fig = go.Figure()
        fig.add_trace(go.Scatter(
            x=sel.lips, y=sel.nrw_value_rm, mode="markers",
            marker=dict(size=np.sqrt(sel.production_m3 / sel.production_m3.max()) * 30 + 7,
                        color=T.BLUE, opacity=0.75,
                        line=dict(color=T.SURFACE, width=2)),
            name="Plant",
            customdata=np.stack([sel.plant, sel.district, sel.lips_rank], -1),
            hovertemplate=("<b>%{customdata[0]}</b> · %{customdata[1]}<br>"
                           "LIPS  %{x:.1f}  (rank %{customdata[2]})<br>"
                           "Value at stake  RM %{y:,.0f}<extra></extra>")))
        fig.update_layout(
            title="Forgone revenue against LIPS", height=380,
            xaxis=dict(title="LIPS (0–100)"),
            yaxis=dict(title="Forgone revenue (RM, log scale)", type="log",
                       dtick=1, minor=dict(showgrid=False)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)

        st.markdown("###### Cost recovery")
        marg = sel.nsmallest(min(10, n_plants), "operating_margin_pct") \
                  .sort_values("operating_margin_pct", ascending=False)
        fig = go.Figure(go.Bar(
            x=marg.operating_margin_pct, y=marg.plant, orientation="h",
            marker=dict(color=[T.CRITICAL if v < 0 else T.BLUE
                               for v in marg.operating_margin_pct],
                        line=dict(color=T.SURFACE, width=2)),
            text=[f"{v:.0f}%" for v in marg.operating_margin_pct],
            textposition="outside", textfont=dict(size=10.5, color=T.INK_2),
            hovertemplate="<b>%{y}</b><br>Operating margin  %{x:.1f}%<extra></extra>"))
        fig.add_vline(x=0, line=dict(color=T.BASELINE, width=1.5))
        fig.update_layout(
            title="Weakest operating margins", height=330, bargap=0.3,
            xaxis=dict(title="Billed revenue less operating cost (%)",
                       ticksuffix="%"),
            yaxis=dict(title=None, tickfont=dict(size=10.5)))
        st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
        st.markdown('<div class="caption">Operating margin here covers energy, '
                    'chemicals and maintenance only. Staff, capital charges, '
                    'depreciation and overhead are not in the dataset, so these '
                    'are not full-cost margins.</div>', unsafe_allow_html=True)
...
with TAB_METHOD:
    ...
    with c1:
        ...
        st.markdown('<div class="caption">Every published figure was '
                    'recomputed from its components. Nothing was silently '
                    'corrected — discrepancies are reported for PAIP to '
                    'adjudicate.</div>', unsafe_allow_html=True)
        q = quality.copy()
        q["status"] = np.where(q.max_abs_deviation <= 1, "✓ consistent", "⚠ review")
        st.dataframe(q, width='stretch', hide_index=True,
                     column_config={
                         "check": "Check",
                         "max_abs_deviation": st.column_config.NumberColumn(
                             "Max deviation", format="%.4f"),
                         "status": "Status"})
        st.markdown(
            '<div class="caption">Residuals of up to 1 m³ on the loss split and '
            '0.05 pp on published percentages are rounding in the source '
            'workbook, not error. <b>No negative NRW records and no month where '
            'billed volume exceeds production were found</b> — the measurement '
            'anomalies anticipated in the proposal do not occur in this '
        ...
        st.dataframe(missing, width='stretch', hide_index=True,
                     column_config={
                         ...
                         "pct_of_rows": st.column_config.NumberColumn(
                             "% of records", format="%.2f%%")})
        st.markdown('<div class="caption">Only two columns have gaps, both '
                    'sensor-derived. They are excluded pairwise from the '
                    'correlations that use them and are not LIPS inputs, so no '
                    'imputation was performed.</div>', unsafe_allow_html=True)

    with c2:
        st.markdown("#### LIPS specification")
        spec = pd.DataFrame([
            {"Component": LIPS_COMPONENTS[k_][0], "Weight %": v,
             "Rationale": LIPS_COMPONENTS[k_][1]}
            for k_, v in weights.items()])
        spec["Weight %"] = (spec["Weight %"] / max(spec["Weight %"].sum(), 1) * 100).round(1)
        st.dataframe(spec, width='stretch', hide_index=True)
        st.markdown(
            '<div class="caption">Each component is converted to a 0–100 '
            'percentile rank within the filtered cohort, then combined on the '
            'weights above. Percentile rank was chosen over min–max scaling '
            'because plant size is heavily right-skewed. Weights are a '
            'management judgement, not an estimated parameter — the sidebar '
            'sliders exist so the ranking can be stress-tested against '
            'alternative priorities.</div>', unsafe_allow_html=True)
...
<b>Target</b> monthly NRW % · <b>Records</b> {ml_metrics['n_records']:,} ·
<b>Features</b> {ml_metrics['n_features']} ·
<b>Selected model</b> {ml_metrics['selected_model']} (chosen on grouped CV) ·
<b>Seed</b> {ml_metrics['seed']}

<b>Leakage control is the central safeguard.</b> NRW is defined as production
minus billed volume, so any feature carrying billed volume encodes the target
algebraically. Two source columns are exactly this trap and are excluded:
<code>consumption_per_capita</code> (billed ÷ population) and
<code>revenue_per_account</code> (billed × tariff ÷ accounts). Both were
confirmed numerically, not assumed. All NRW-derived measures are excluded too.
Features that divide by <i>production</i> — capacity utilisation, energy
intensity, cost per m³ — are retained and safe. The exclusion list is asserted
at runtime and re-checked in <code>verify.py</code>.

<b>Validation is grouped by plant.</b> Static characteristics are near-constant
within a plant, so a random split would let the model memorise each plant's own
loss level and the residual would collapse. GroupKFold forces every prediction
to come from a model that never saw that plant.

<b>Imputation sits inside the pipeline</b>, so fold medians are learned on
training data alone.

</div>""", unsafe_allow_html=True)

        st.markdown("#### Limitations")
        n_history_months = int(monthly.groupby("plant").date.nunique().median())
        st.markdown(f"""
<div class="caption">

<b>The model has no ground truth.</b> This dataset records no repairs, no
inspections and no confirmed defects, so nothing here is a trained failure
classifier and it is not presented as one. It models expected loss given plant
characteristics; a large residual means "this plant is unusual for its type",
which is a lead to investigate, not a diagnosis.

<b>Unexplained loss is only as good as the feature set.</b> A plant may lose
more than predicted because of terrain, soil, water hammer or historic
construction quality — none of which are in the data. The residual bounds where
to look; it does not identify the cause.

<b>Cluster separation is weak</b> (silhouette ≈ 0.20). The estate varies
continuously rather than falling into discrete failure types, so the archetypes
are a communication aid, not evidence of real categories.


<b>Repair cost is not in the dataset.</b> LIPS ranks by recoverable <i>volume</i>
weighted by condition, not by cost per m³ recovered. A plant losing 40 million m³
across a dispersed ageing network may cost far more per m³ to fix than one losing
2 million through a single identifiable main. Leak concentration (NRW per km) is
included as a partial proxy, but it is not a substitute for repair costing.

<b>The physical/commercial split is a published apportionment.</b> PAIP reports a
physical loss share per plant; this dashboard uses it as given. It is not derived
here from a formal IWA water-balance audit, and its own accuracy is not testable
from these fields.

<b>Correlation across plants is not causal.</b> Plants differ in network length,
terrain, age and pressure at the same time. The driver scatter plots describe
association only.

<b>The history is short.</b> Trend and year-specific event cannot be fully
separated over {YEAR_SPAN}, and per-plant anomaly detection rests on a median of
{n_history_months} monthly observations per plant. This limitation eases with
every annual refresh.

<b>Financial figures are tariff-based, not full-cost.</b> Forgone revenue values
lost water at the published average tariff. Operating cost covers energy,
chemicals and maintenance only — staff, capital charges and depreciation are not
in the dataset, so operating margins are overstated relative to full cost
recovery.

</div>""", unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("#### Scope note — dataset against original proposal")
    st.markdown(T.callout(
        "The dataset supplied for this build differs materially from the one "
        "the proposal described, and the dashboard was extended accordingly. "
        f"The proposal anticipated 888 plant-month records for one year with "
        "no cost or tariff fields, no physical/commercial split, and negative "
        "NRW records requiring flagging. The delivered dataset contains "
        f"<b>{len(monthly):,} plant-month records across {YEAR_SPAN}</b>, includes "
        "<b>tariff and full operating cost</b> fields, carries an explicit "
        "<b>physical versus commercial loss split</b>, and contains "
        "<b>no negative NRW records</b>. Financial and multi-year analysis is "
        "therefore in scope where the proposal had ruled it out, and the "
        "negative-record limitation does not arise.", "warn"),
        unsafe_allow_html=True)

    c5, c6 = st.columns([1, 1])
    with c5:
        st.markdown("###### Plant crosswalk")
        st.markdown('<div class="caption">Plant names are unique across the '
                    'estate — no name appears in more than one district — so '
                    'the crosswalk is a direct one-to-one mapping.</div>',
                    unsafe_allow_html=True)
        st.dataframe(crosswalk, width='stretch', hide_index=True,
                     height=340)
        st.download_button("Download crosswalk (CSV)",
                           crosswalk.to_csv(index=False),
                           "paip_plant_crosswalk.csv", "text/csv")
    with c6:
        st.markdown("###### Dataset summary")
        summary = pd.DataFrame({
            "Item": ["Plant-month records", "Plants", "Districts", "Regions",
                     "Years covered", "Columns in source", "Derived measures",
                     "Records in current selection"],
            "Value": [f"{len(monthly):,}", f"{monthly.plant.nunique()}",
                      f"{monthly.district.nunique()}",
                      f"{monthly.region.nunique()}",
                      f"{monthly.year.min()}–{monthly.year.max()}",
                      "50", "12", f"{len(msel):,}"]})
        st.dataframe(summary, width='stretch', hide_index=True)
        st.markdown("###### Tidy dataset")
        st.download_button("Download plant-month dataset (CSV)",
                           monthly.to_csv(index=False),
                           "paip_nrw_plant_month.csv", "text/csv")
        st.download_button("Download plant-year dataset (CSV)",
                           yearly.to_csv(index=False),
                           "paip_nrw_plant_year.csv", "text/csv")
...
# ==========================================================================
with TAB_DATA:
    st.markdown("#### Adding next year's data")
    st.markdown(T.callout(
        "The dashboard reads whatever years are present — nothing is hard-coded "
        "to a particular year. To bring in a new one, drop the PAIP export here "
        "(or into <code>data/raw/</code>) and rebuild. Validation runs first, "
        "and a rebuild that fails validation leaves the current build "
        "untouched."), unsafe_allow_html=True)
    ...
    with c1:
        ...
        rows = [
            {"Item": "Years covered", "Value": YEAR_SPAN},
            {"Item": "Plant-month records", "Value": f"{len(monthly):,}"},
            {"Item": "Plants", "Value": f"{monthly.plant.nunique()}"},
            {"Item": "Districts", "Value": f"{monthly.district.nunique()}"},
            {"Item": "Models fitted for", "Value": str(ML_YEAR) if HAS_ML else "—"},
        ]
        if coverage is not None:
            incomplete = coverage[~coverage.complete]
            rows.append({"Item": "Incomplete years",
                         "Value": ", ".join(incomplete.label) if len(incomplete)
                                  else "none"})
        ...
        if coverage is not None:
            ...
            cov = coverage.copy()
            fig = go.Figure(go.Bar(
                x=cov.year.astype(int).astype(str), y=cov.months,
                marker=dict(color=[T.BLUE if c else T.SERIOUS
                                   for c in cov.complete],
                            line=dict(color=T.SURFACE, width=2)),
                text=[f"{m}/12" for m in cov.months], textposition="outside",
                textfont=dict(size=11, color=T.INK_2),
                customdata=cov.records,
                hovertemplate=("<b>%{x}</b><br>%{y} of 12 months<br>"
                               "%{customdata:,} records<extra></extra>")))
            fig.add_hline(y=12, line=dict(color=T.BASELINE, width=1, dash="dot"))
            fig.update_layout(
                title="Months observed per year", height=300, bargap=0.5,
                showlegend=False, xaxis=dict(title=None),
                yaxis=dict(title="Months", range=[0, 14],
                           tickmode="array", tickvals=[0, 3, 6, 9, 12]))
            st.plotly_chart(fig, width='stretch', config=PLOT_CFG, theme=None)
            st.markdown(
                '<div class="caption">Amber marks a year still in progress. '
                'Partial years are kept and labelled rather than hidden; '
                'volumes are annualised wherever years are compared.</div>',
                unsafe_allow_html=True)

    with c2:
        st.markdown("###### Validate a new export")
        st.markdown(
            '<div class="caption">Upload a PAIP CSV to check it <b>before</b> '
            'committing to a rebuild. This only inspects the file — nothing is '
            'written and the current build is not touched.</div>',
            unsafe_allow_html=True)
        up = st.file_uploader("PAIP export (CSV)", type=["csv"],
                              label_visibility="collapsed")
        if up is not None:
            tmp = DATA / "_upload_check.csv"
            tmp.write_bytes(up.getbuffer())
            try:
                from dataloader import ingest as _ingest
                from dataloader import year_completeness as _yc
                newdf, rep = _ingest(tmp, known_plants=crosswalk.plant.tolist(),
                                     strict=False)
                if rep.ok and not rep.warnings:
                    st.success("Validation passed with no warnings.")
                elif rep.ok:
                    st.warning(f"Validation passed with {len(rep.warnings)} "
                               f"warning(s). Review before rebuilding.")
                else:
                    st.error(f"Validation FAILED with {len(rep.errors)} "
                             f"error(s). This file cannot be used as-is.")
                st.code(rep.text(), language="text")

                cov_new = _yc(newdf)
                st.markdown("###### Years in the uploaded file")
                st.dataframe(cov_new[["year", "months", "records", "complete"]],
                             width='stretch', hide_index=True)

                new_years = sorted(set(cov_new.year.astype(int)) - set(YEARS))
                if new_years:
                    st.info(f"This file adds year(s) not in the current build: "
                            f"{', '.join(map(str, new_years))}")
                if rep.ok:
                    st.markdown(f"""
<div class="caption">
To apply it, save the file into <code>data/raw/</code> and run:

<pre>python refresh.py --add path/to/{up.name}</pre>

That copies it in, rebuilds the tidy dataset, refits the models and runs the
{49} verification checks. Restart the dashboard afterwards to pick up the new
build. Existing artefacts are backed up to <code>data/_backup/</code> first, so
a bad refresh can be rolled back.
</div>""", unsafe_allow_html=True)
            except Exception as exc:
                st.error("The file could not be read at all.")
                st.code(str(exc), language="text")
            finally:
                tmp.unlink(missing_ok=True)

    st.markdown("---")
    st.markdown("#### Refresh procedure")
    st.markdown("""
<div class="caption">

**1 — Put the file where the loader can see it.** Either replace the existing
export in <code>data/raw/</code> with a new workbook covering every year, or add
a per-year file alongside it. The loader auto-detects which pattern you used,
concatenates everything in the folder, and de-duplicates on plant and month
keeping the <i>last</i> occurrence — so a corrected re-issue of an old year
supersedes the original instead of double-counting.

**2 — Dry run first.** <code>python refresh.py --dry-run</code> validates and
writes nothing. Fix anything reported as an ERROR; WARNINGS are advisory.

**3 — Rebuild.** <code>python refresh.py</code> runs clean → train → verify in
order. Clean failing stops everything, because training on a malformed extract
produces confident nonsense. Verify failing means the rebuilt artefacts disagree
with the raw data — do not publish that build.

**4 — Restart the dashboard.** Streamlit caches the data on load, so the new
build appears after a restart (or press **R** to rerun and clear the cache).

</div>""", unsafe_allow_html=True)

    st.markdown("#### What the loader checks")
    checks_doc = pd.DataFrame([
        {"Check": "Required columns present", "Severity": "Error",
         "Note": "date, plant, district, production, billed — the rest is derivable"},
        {"Check": "Dates parse unambiguously", "Severity": "Error",
         "Note": "Day-first tried before month-first, matching PAIP's format"},
        {"Check": "Production is positive", "Severity": "Error",
         "Note": "Zero production makes the loss rate undefined"},
        {"Check": "Plant maps to one district", "Severity": "Error",
         "Note": "Otherwise the crosswalk is ambiguous"},
        {"Check": "NRW = production − billed", "Severity": "Warning",
         "Note": "Published figure kept, discrepancy reported"},
        {"Check": "Negative NRW", "Severity": "Warning",
         "Note": "Retained and flagged, never silently corrected"},
        {"Check": "Duplicate plant-months", "Severity": "Warning",
         "Note": "Last occurrence wins; treated as a correction"},
        {"Check": "Year gaps in coverage", "Severity": "Warning",
         "Note": "Year-on-year comparisons skip missing years"},
        {"Check": "New or vanished plants", "Severity": "Note",
         "Note": "A vanished plant is usually a rename — worth checking"},
        {"Check": "Incomplete years", "Severity": "Note",
         "Note": "Kept, labelled, and annualised where years are compared"},
    ])
    st.dataframe(checks_doc, width='stretch', hide_index=True)
